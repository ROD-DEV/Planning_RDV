...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {primaryStage.setOnCloseRequest ( (ev)-> {
			try{
			...
			// Main.class.getResource("../vue.VueRendezVous.fxml") retourne
			// l'URL du fichier fxml à charger
			// Indique ainsi au chargeur l'emplacement du fichier à charger
			loader.setLocation(Main.class.getResource("../agenda/vue/VueRendezVous.fxml"));
			// Charge l'objet correspondant au fichier comme objet racine
			GridPane rootLayout = (GridPane) loader.load();
			Scene scene = new Scene(rootLayout);
			primaryStage.setScene(scene);
			primaryStage.setTitle("Agenda personnel");
			// Insertion d'une image à gauche dans la barre de titre de la
			// fenêtre
